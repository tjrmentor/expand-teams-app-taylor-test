...
const awsSecrets = await getAWSSecrets(WEBHOOK_TOKEN_SECRET_NAME, PRIVATE_KEY_SECRET_NAME);
const APP_ID = 414851;
const app = new App({
    appId: APP_ID,
...
async function getAWSSecrets(webhookSecretName, privateKeySecretName) {
    const client = new SecretsManagerClient({
        region: "us-east-1",
    });
    let webhookSecretResponse, privateKeyResponse;
    try {
        webhookSecretResponse = await client.send(new GetSecretValueCommand({
            SecretId: webhookSecretName,
            VersionStage: "AWSCURRENT", // VersionStage defaults to AWSCURRENT if unspecified
        }));
        privateKeyResponse = await client.send(new GetSecretValueCommand({
